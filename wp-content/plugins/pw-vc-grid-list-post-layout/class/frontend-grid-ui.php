...
if (!class_exists('VC_PW_grid')) {
	class VC_PW_grid  {
		...
		function __construct(
			...
			$this->pw_readmore_translate = ($pw_readmore_translate=='' ? __("Read More",__PW_POST_LAYOUT_TEXTDOMAN__):$pw_readmore_translate);
		...
		function pw_front_end()
		{
			...
			foreach($query as $query_part)
			{
				...
				switch($q_part[0])
				{
					...
					case 'size':
						$query_posts_per_page=(strtolower($q_part[1])=='all' ? -1:$q_part[1]);
					break;
					...
					case 'categories':
						...
						if(count($query_cat_not_in)>0)
						{
							$query_tax_query[]=array(
								'taxonomy' => 'category',
								'field'    => 'id',
								'terms'    => $query_cat_not_in,
								'operator' => 'NOT IN',
								'include_children' => true
							);
						}else if(count($query_cat_in)>0)
						{
							$query_tax_query[]=array(
								'taxonomy' => 'category',
								'field'    => 'id',
					...
					case 'tax_query':
						...
						foreach ( $all_tax as $tax ) {
							...
							foreach($query_taxs as $taxs)
							{
								...
								$termmm=get_term_by('id', $taxs, $tax);

								if(is_object($termmm)){

									$tax_name=$termmm->taxonomy;
			...
			while ( $my_query->have_posts() ) {
				...
				foreach ( $all_tax as $post_taxs ) {										
					...
					$include_cat_ids[$post_taxs]=array();
							...
							if(is_array($taxs) && count($taxs)>1)
							{
								foreach($taxs as $tax)
								{
									...
									<div class="pl-drop_down wf" style="z-index:'.$z_index.';">
										<span class="pl-anim150">'.__($taxonomy->label,__PW_POST_LAYOUT_TEXTDOMAN__).'</span>
										<ul class="pl-anim250">
									...
									foreach ($terms as $term){
										if(!is_numeric($term->name)) $iss_numeric=false;
										$final_terms[$term->name]=$term->term_id;
									}
									if($iss_numeric)
										ksort( $final_terms, SORT_NUMERIC );
									foreach ($final_terms as $key=>$term){
										if(!in_array("-".$term,$query_not_in))
											$output.='<li  data-dimension="grid_filter_'.str_replace('-','_',$tax).'" data-filter="'.strtolower($term).'" >'.$key.'</li>';
...
//												}
							...
							}else if( $this->pw_taxonomy != '' && isset($main_include_cat_ids[$this->pw_taxonomy]))
							{
								...
								<div class="pl-drop_down wf">
									<span class="pl-anim150">'.__($tax->label,__PW_POST_LAYOUT_TEXTDOMAN__).'</span>
									<ul class="pl-anim250">
								...
								foreach ($final_terms as $key=>$term){
									if(!in_array("-".$term,$query_not_in))
										$output.='<li  data-dimension="grid_filter_'.str_replace('-','_',$this->pw_taxonomy).'" data-filter="'.strtolower($term).'" >'.$key.'</li>';
...
//										}
						...
						if($this->pw_grid_order_by=='yes')
						{
							...
							$output .= '
							<div class="pl-meta">
								<div class="meta-title">'.__(ucwords ( str_replace('_',' ',$order_by) ),'js_composer').'</div>
						...
						$post_orders = explode(',',$this->pw_teasr_layout_img);
							...
							foreach ( $post_orders as $order ){
								...
								case 'text':
											...
											$output .= '<div class="pl-detailcnt">
														<p class="pl-text center-txt">'. $content.'</p>';
														if($this->pw_grid_show_num_comment=='no')
														{
															$output .= '
														<div class="pl-postcomment"><a href="'. get_comments_link() .'"><i class="fa fa-comments"></i>'.get_comments_number( '0', '1', '% responses' ).'</a></div>';
